...
const getMoodLabel = pct => pct<=0?"No Budget Set":pct<50?"Great Saving!":pct<85?"On Track":pct<100?"Careful!":"Over Budget!";
...
const CAT_KEYWORDS={Food:["food","eat","lunch","dinner","breakfast","restaurant","cafe","coffee","snack","grocery","khana","nashta","chai","sabji","fruits","vegetable","bhojan","khaanu","jamu","roti","dal","rice","milk","pizza","burger","biryani","thali","sweets","juice"],Transport:["transport","travel","auto","cab","taxi","uber","ola","bus","train","metro","petrol","diesel","fuel","rickshaw","bike","ticket","fare","gaadi","car","scooter","railway","flight","toll","parking"],Shopping:["shopping","clothes","shirt","pant","dress","shoes","amazon","flipkart","order","buy","purchase","market","bazaar","mall","online","kapda","jota","saree","jewellery","watch","bag","mobile","laptop"],Entertainment:["movie","cinema","show","concert","game","gaming","netflix","hotstar","youtube","ott","fun","play","masti","tamasha","natak","party","birthday","celebration","spotify","music","cricket","football","match"],Health:["health","medicine","doctor","hospital","clinic","pharmacy","medical","tablet","gym","fitness","checkup","dava","dawai","aushadh","blood","xray","scan","yoga","vitamin","supplement"],Bills:["bill","electricity","wifi","internet","mobile","recharge","phone","water","gas","rent","light","bijli","paani","ghar","dth","cable","broadband","insurance","emi","loan","tax"],Education:["education","school","college","tuition","course","book","fees","class","study","exam","abhyas","shiksha","coaching","notes","pen","paper","uniform","library","training"]};
...
/* ── STYLE HELPERS (module-level, pure functions) ─────────────────────── */
const mkCard=(T,extra={})=>({background:T.card,backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",border:`1px solid ${T.bdrSub}`,borderRadius:16,padding:18,boxShadow:T.isDark?"0 4px 24px rgba(0,0,0,.35)":"0 2px 16px rgba(0,0,0,.07),0 1px 0 rgba(255,255,255,.9) inset",...extra});
const mkBtn=(extra={})=>({border:"none",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:600,borderRadius:12,transition:"all .2s",...extra});

/* ═══════════════════════════════════════════════════════════════════════
   VIEW COMPONENTS — defined OUTSIDE App so React never remounts them
   on re-render. This fixes all input focus / scroll bugs.
═══════════════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════ HOME VIEW ══════════════════════ */
function HomeView({T,expenses,thisMonth,total,upiAmt,cashAmt,pct,isOver,curTarget,catData,overspendAlert,setOverspendAlert,undoEntry,undoDelete,exportMode,setExportMode,exportCSV,setTab,deleteEntry,getCatById,recurringTemplates}){
  const card=(e={})=>mkCard(T,e);
  const btn=(e={})=>mkBtn(e);
  const now=new Date();
  const moodColor = getMoodColor(curTarget>0?pct:0,T);
  const moodEmoji = getMoodEmoji(curTarget>0?pct:0);
  const moodLabel = getMoodLabel(curTarget>0?pct:0);
  const isDanger  = curTarget>0 && pct>=100;
  const isSaving  = curTarget>0 && pct<50;

  return(
    <div style={{padding:"0 16px 124px"}}>
      {/* Dynamic mood banner */}
      {curTarget>0&&(
        <div style={{marginBottom:12,padding:"10px 16px",borderRadius:14,
          background:isDanger?`${T.a4}18`:isSaving?"rgba(34,197,94,.12)":"transparent",
          border:`1px solid ${isDanger?T.a4:isSaving?"#22c55e":"transparent"}`,
          display:"flex",alignItems:"center",justifyContent:"space-between",
          animation:isSaving?"celebrate 2s ease-in-out infinite":"none",
          transition:"all .5s ease"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:22}}>{moodEmoji}</span>
            <div>
              <div style={{fontWeight:700,color:moodColor,fontSize:14}}>{moodLabel}</div>
              <div style={{fontSize:12,color:T.muted}}>{curTarget>0?`${(100-Math.min(pct,100)).toFixed(0)}% budget remaining`:"Set a budget to track spending"}</div>
            </div>
          </div>
          {isSaving&&<span style={{fontSize:11,fontWeight:700,padding:"4px 10px",borderRadius:99,background:"rgba(34,197,94,.18)",color:"#22c55e"}}>🏆 SAVER</span>}
          {isDanger&&<span style={{fontSize:11,fontWeight:700,padding:"4px 10px",borderRadius:99,background:`${T.a4}20`,color:T.a4}}>OVER</span>}
        </div>
      )}
      ...
      {/* Hero card — color changes with spending */}
      <div style={{...card({background:isDanger?`${T.a4}10`:isSaving?"rgba(34,197,94,.08)":T.gradSoft,borderColor:isDanger?T.a4:isSaving?"#22c55e":T.border,marginBottom:12,padding:"22px",position:"relative",overflow:"hidden",transition:"all .5s ease"})}}>
        <div style={{position:"absolute",top:-40,right:-40,width:150,height:150,borderRadius:"50%",background:`radial-gradient(circle,${moodColor}18,transparent)`,transition:"background .5s"}}/>
        <div style={{fontSize:12,color:T.sub,marginBottom:6}}>{now.toLocaleDateString("en-IN",{month:"long",year:"numeric"})}</div>
        <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:40,fontWeight:700,letterSpacing:-1,lineHeight:1.1,color:moodColor,transition:"color .5s"}}>{fmt(total)}</div>
        ...
        {curTarget>0&&(
          <div style={{marginTop:16}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
              ...
              <span style={{fontSize:12,fontWeight:700,color:moodColor,transition:"color .5s"}}>{pct.toFixed(1)}%</span>
            </div>
            <div style={{height:8,borderRadius:99,background:T.isDark?"rgba(255,255,255,.08)":"rgba(0,0,0,.07)",overflow:"hidden"}}>
              <div style={{height:"100%",width:`${pct}%`,borderRadius:99,transition:"width .7s ease, background .5s ease",background:moodColor,boxShadow:`0 0 10px ${moodColor}44`}}/>
            </div>
          </div>
        )}
        {!curTarget&&<div style={{marginTop:12,fontSize:12,color:T.muted}}>No budget set — <span onClick={()=>setTab("budget")} style={{color:T.a1,cursor:"pointer",textDecoration:"underline"}}>set one in Budget tab</span></div>}
      </div>

      {/* Stats */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:12}}>
        {[{l:"UPI",v:upiAmt,c:T.upi,ico:"💳"},{l:"Cash",v:cashAmt,c:T.cash,ico:"💵"},{l:curTarget>0?(isOver?"Overrun":"Left"):"Spent",v:curTarget>0?Math.abs(curTarget-total):total,c:moodColor,ico:getMoodEmoji(curTarget>0?pct:0),p:isOver?"+":""}].map(s=>(
          <div key={s.l} style={{...card({padding:"13px 8px",textAlign:"center"})}}><div style={{fontSize:11,color:T.muted,marginBottom:5}}>{s.ico} {s.l}</div><div style={{fontWeight:700,fontSize:12,color:s.c,fontFamily:"'JetBrains Mono',monospace",lineHeight:1.2,wordBreak:"break-all",transition:"color .5s"}}>{s.p}{fmt(s.v)}</div></div>
        ))}
      </div>
      ...
      {recurringTemplates.filter(r=>r.active).length>0&&(
        <div style={{...card({marginBottom:14,borderColor:T.border,background:T.gradSoft})}}>
          ...
          {recurringTemplates.filter(r=>r.active).map(r=>(
            <div key={r.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:13,color:T.sub,marginBottom:6}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                <span style={{fontWeight:600,color:T.text,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(r.amount)}</span>
      ...
      {exportMode&&<div style={{...card({marginBottom:14,borderColor:T.border})}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>{["week","month","year"].map(v=><button key={v} onClick={()=>exportCSV(v)} style={{...btn({padding:"11px 4px",border:`1.5px solid ${T.border}`,background:`${T.a1}10`,color:T.a1,fontSize:12,textAlign:"center",borderRadius:12})}}>{v==="week"?"This Week":v==="month"?"This Month":"This Year"}</button>)}</div>
      ...
      {expenses.slice(0,40).map(e=>{
        const cat=getCatById(e.category); const d=new Date(e.date);
        return(
          <div key={e.id} style={{...card({marginBottom:8,display:"flex",alignItems:"center",gap:12,padding:"12px 14px"})}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:11,color:T.muted,display:"flex",gap:6,marginTop:3}}><span>{cat.name}</span><span>·</span><span>{d.toLocaleDateString("en-IN",{day:"numeric",month:"short"})}</span>{e.recurring&&<span style={{color:T.a3,fontWeight:600}}>🔁</span>}</div>
            </div>
...
/* ══════════════════════════════════════ ADD VIEW ═══════════════════════ */
function AddView({T,form,setForm,addExpense,allCats,getCatById,showCatManager,setShowCatManager,customCats,addCustomCat,deleteCustomCat,newCatName,setNewCatName,newCatEmoji,setNewCatEmoji,newCatColor,setNewCatColor,voiceLang,setVoiceLang,voiceText,transText,listening,vLoad,startVoice,stopVoice,showVHelp,setShowVHelp,recRef,recurringTemplates,setRecurringTemplates,showToast,persist,user,expenses,budgets,monthlyTargets,isDark,pinVal,customCatsRaw}){
  const card=(e={})=>mkCard(T,e);
  const btn=(e={})=>mkBtn(e);

  /* OCR state — local so it doesn't trigger App re-render */
  const [ocrLoading,setOcrLoading] = useState(false);
  const [ocrStatus,setOcrStatus]   = useState("");
  const fileInputRef = useRef(null);

  /* Recurring toggle for current add form */
  ...
  const scanReceipt = async (file) => {
    if(!file)return;
    setOcrLoading(true); setOcrStatus("Loading OCR engine…");
    try{
      if(!window.Tesseract){
      ...
      /* extract amount — look for total/amount/rs patterns */
      const patterns=[
        /(?:total|grand total|amount|rs\.?|₹|inr)\s*[:=]?\s*(\d[\d,]*(?:\.\d{1,2})?)/i,
        /(\d[\d,]+(?:\.\d{1,2})?)\s*(?:total|rs|₹)/i,
        /(\d{2,6}(?:\.\d{1,2})?)/,
      ];
      let found="";
      ...
      else{setOcrStatus("No amount found — please enter manually.");showToast("Could not find amount — enter manually","warn");}
    }catch(e){setOcrStatus("Scan failed.");showToast("OCR failed — enter manually","warn");}
    setOcrLoading(false);
  };

  const handleRecurringSave = async () => {
    if(!form.amount||parseFloat(form.amount)<=0){showToast("Enter an amount first.","warn");return;}
    const cat=getCatById(form.category);
    const template={id:Date.now(),name:form.description||cat.name,amount:parseFloat(form.amount),category:form.category,type:form.type,dayOfMonth:recurDay,emoji:cat.emoji,active:true};
    ...
    showToast(`🔁 Recurring template "${template.name}" saved!`);
  };
    ...
    <div style={{padding:"0 16px 124px"}}>
      ...
      <div style={{...card({marginBottom:12,padding:"20px"})}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <button onClick={()=>fileInputRef.current?.click()} disabled={ocrLoading} style={{...btn({fontSize:11,padding:"5px 12px",borderRadius:8,border:`1px solid ${T.border}`,background:`${T.a3}10`,color:T.a3,display:"flex",alignItems:"center",gap:5})}}>
              <span>📷</span>{ocrLoading?"Scanning…":"Scan Receipt"}
            </button>
            <input ref={fileInputRef} type="file" accept="image/*" capture="environment" style={{display:"none"}} onChange={e=>e.target.files[0]&&scanReceipt(e.target.files[0])}/>
          </div>
        </div>
      ...
      <div style={{...card({marginBottom:12})}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <button onClick={()=>setMakeRecurring(p=>!p)} style={{...btn({padding:"7px 14px",borderRadius:99,border:`1.5px solid ${makeRecurring?T.a3:T.bdrSub}`,background:makeRecurring?`${T.a3}15`:"transparent",color:makeRecurring?T.a3:T.muted,fontSize:13})}}>
            {makeRecurring?"ON ✓":"OFF"}
          </button>
        </div>
        {makeRecurring&&(
          <div style={{marginTop:14,animation:"fadeIn .2s ease"}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
              {[1,5,10,15,20,25,28].map(d=>(
                <button key={d} onClick={()=>setRecurDay(d)} style={{...btn({padding:"7px 12px",borderRadius:99,border:`1.5px solid ${recurDay===d?T.a3:T.bdrSub}`,background:recurDay===d?`${T.a3}15`:"transparent",color:recurDay===d?T.a3:T.muted,fontSize:12})}}>
                  {d}{d===1?"st":d===2?"nd":d===3?"rd":"th"}
                </button>
              ))}
            </div>
            <button onClick={handleRecurringSave} style={{...btn({width:"100%",padding:"12px",borderRadius:12,background:`${T.a3}15`,border:`1.5px solid ${T.a3}`,color:T.a3,fontSize:14})}}>
      ...
      <div style={{...card({marginBottom:16})}}>
        ...
        {showVHelp&&<div style={{marginBottom:14,padding:"14px",borderRadius:12,background:T.isDark?"rgba(255,255,255,.04)":"rgba(0,0,0,.04)",border:`1px solid ${T.bdrSub}`,animation:"fadeIn .2s ease"}}>
          ...
          <div style={{marginTop:10,padding:"8px 12px",borderRadius:8,background:`${T.a3}10`,border:`1px solid ${T.a3}20`}}><div style={{fontSize:11,color:T.a3,lineHeight:1.6}}>💡 Google Translate auto-detects language · Say: Amount → Item → UPI or Cash</div></div>
        </div>}
        <button onClick={listening?stopVoice:startVoice} style={{...btn({width:"100%",padding:"13px",borderRadius:12,border:`1.5px solid ${listening?T.a4:T.border}`,background:listening?`${T.a4}10`:`${T.a1}08`,color:listening?T.a4:T.a1,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",gap:10})}}>
      ...
      {showCatManager&&(
        <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,.6)",backdropFilter:"blur(6px)",display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"16px"}} onClick={e=>{if(e.target===e.currentTarget)setShowCatManager(false);}}>
          <div style={{...mkCard(T,{width:"min(480px,100%)",padding:"24px",maxHeight:"85vh",overflowY:"auto",borderRadius:20,background:T.isDark?"#13131f":"#fff"})}}>
            ...
            <div style={{...mkCard(T,{marginBottom:16,padding:"16px",borderColor:T.border})}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>{CAT_COLORS.map(c=><button key={c} onClick={()=>setNewCatColor(c)} style={{width:28,height:28,borderRadius:"50%",background:c,border:`3px solid ${newCatColor===c?"#fff":"transparent"}`,cursor:"pointer",outline:newCatColor===c?`2px solid ${c}`:"none",transition:"all .15s"}}/>)}</div>
            ...
            {customCats.length>0&&<>{<div style={{fontSize:12,color:T.muted,fontWeight:600,marginBottom:10}}>YOUR CUSTOM CATEGORIES</div>}{customCats.map(c=><div key={c.id} style={{...mkCard(T,{marginBottom:8,padding:"12px 14px",display:"flex",alignItems:"center",gap:12})}}><div style={{width:38,height:38,borderRadius:10,background:`${c.color}15`,border:`1px solid ${c.color}30`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{c.emoji}</div><div style={{flex:1}}><div style={{fontWeight:600,color:T.text,fontSize:14}}>{c.name}</div></div><button onClick={()=>deleteCustomCat(c.id)} style={{...mkBtn({padding:"6px 12px",borderRadius:8,border:`1px solid ${T.a4}30`,background:`${T.a4}08`,color:T.a4,fontSize:12})}}>Remove</button></div>)}</>}
            <div style={{fontSize:12,color:T.muted,marginTop:8,marginBottom:8,fontWeight:600}}>DEFAULT CATEGORIES</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{DEFAULT_CATS.map(c=><div key={c.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",borderRadius:10,background:T.input}}><span style={{fontSize:16}}>{c.emoji}</span><span style={{fontSize:13,color:T.muted}}>{c.name}</span></div>)}</div>
            <button onClick={()=>setShowCatManager(false)} style={{...mkBtn({width:"100%",padding:"13px",marginTop:16,border:`1px solid ${T.bdrSub}`,background:"transparent",color:T.text,fontSize:14})}}>Done</button>
          </div>
        </div>
      )}
    </div>
...
function ChartsView({T,chartData,period,setPeriod,catData,thisMonth,upiAmt,cashAmt}){
  ...
  const CTipLocal=({active,payload,label})=><CTip active={active} payload={payload} label={label} T={T}/>;
  return(
    <div style={{padding:"0 16px 124px",width:"100%"}}>
      ...
      <div style={{...card({marginBottom:14,padding:"18px 10px 12px 6px"})}}>
        ...
        <ResponsiveContainer width="100%" height={210}><BarChart data={chartData} margin={{top:4,right:6,left:-12,bottom:0}} barCategoryGap="30%"><CartesianGrid strokeDasharray="3 3" stroke={T.gridLine} vertical={false}/><XAxis dataKey="name" tick={{fill:T.muted,fontSize:11}} axisLine={false} tickLine={false}/><YAxis tick={{fill:T.muted,fontSize:10}} axisLine={false} tickLine={false} tickFormatter={v=>v>999?(v/1000).toFixed(1)+"k":v} width={36}/><Tooltip content={<CTipLocal/>} cursor={{fill:T.isDark?"rgba(255,255,255,.04)":"rgba(0,0,0,.04)"}}/><Bar dataKey="UPI" fill={T.upi} radius={[6,6,0,0]} isAnimationActive={false}/><Bar dataKey="Cash" fill={T.cash} radius={[6,6,0,0]} isAnimationActive={false}/></BarChart></ResponsiveContainer>
      </div>
      <div style={{...card({marginBottom:14,padding:"18px 10px 12px 6px"})}}>
        ...
        <ResponsiveContainer width="100%" height={190}><LineChart data={chartData} margin={{top:4,right:6,left:-12,bottom:0}}><CartesianGrid strokeDasharray="3 3" stroke={T.gridLine} vertical={false}/><XAxis dataKey="name" tick={{fill:T.muted,fontSize:11}} axisLine={false} tickLine={false}/><YAxis tick={{fill:T.muted,fontSize:10}} axisLine={false} tickLine={false} tickFormatter={v=>v>999?(v/1000).toFixed(1)+"k":v} width={36}/><Tooltip content={<CTipLocal/>}/><Line type="monotone" dataKey="UPI" stroke={T.upi} strokeWidth={2.5} dot={{fill:T.upi,r:4,strokeWidth:0}} activeDot={{r:7,strokeWidth:0}} isAnimationActive={false}/><Line type="monotone" dataKey="Cash" stroke={T.cash} strokeWidth={2.5} dot={{fill:T.cash,r:4,strokeWidth:0}} activeDot={{r:7,strokeWidth:0}} isAnimationActive={false}/><Legend wrapperStyle={{fontSize:12,color:T.muted,paddingTop:8}}/></LineChart></ResponsiveContainer>
      </div>
      <div style={{...card({marginBottom:14,padding:"18px 16px"})}}>
        ...
        {pieD.length>0?(<><ResponsiveContainer width="100%" height={190}><PieChart><Pie data={pieD} cx="50%" cy="50%" innerRadius={52} outerRadius={82} dataKey="value" paddingAngle={3} isAnimationActive={false}>{pieD.map((d,i)=><Cell key={i} fill={d.color}/>)}</Pie><Tooltip formatter={v=>[fmt(v),"Amount"]} contentStyle={{background:T.isDark?"rgba(10,14,25,.97)":"rgba(255,255,255,.98)",border:`1px solid ${T.border}`,borderRadius:12,fontFamily:"DM Sans"}}/></PieChart></ResponsiveContainer><div style={{display:"flex",flexDirection:"column",gap:8,marginTop:8}}>{pieD.map(d=><div key={d.name} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:8,height:8,borderRadius:"50%",background:d.color,flexShrink:0}}/><span style={{fontSize:13,color:T.muted}}>{d.emoji} {d.name}</span></div><div style={{display:"flex",gap:10,alignItems:"center"}}><div style={{width:70,height:4,borderRadius:99,background:T.isDark?"rgba(255,255,255,.07)":"rgba(0,0,0,.07)",overflow:"hidden"}}><div style={{height:"100%",width:`${(d.value/pieT)*100}%`,background:d.color,borderRadius:99}}/></div><span style={{fontSize:12,fontWeight:700,color:T.text,fontFamily:"'JetBrains Mono',monospace",minWidth:68,textAlign:"right"}}>{fmt(d.value)}</span><span style={{fontSize:11,color:T.muted,minWidth:28,textAlign:"right"}}>{((d.value/pieT)*100).toFixed(0)}%</span></div></div>)}</div></>):<div style={{textAlign:"center",padding:"36px 0",color:T.muted,fontSize:14}}><div style={{fontSize:32,marginBottom:12}}>📊</div>Add expenses to see charts!</div>}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{[{l:"UPI Total",cnt:thisMonth.filter(e=>e.type==="UPI").length,amt:upiAmt,c:T.upi},{l:"Cash Total",cnt:thisMonth.filter(e=>e.type==="Cash").length,amt:cashAmt,c:T.cash}].map(s=><div key={s.l} style={{...card({textAlign:"center",padding:"16px 10px"})}}><div style={{fontSize:11,color:T.muted,marginBottom:6}}>{s.l}</div><div style={{fontWeight:700,fontSize:17,color:s.c,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(s.amt)}</div><div style={{fontSize:11,color:T.muted,marginTop:4}}>{s.cnt} transactions</div></div>)}</div>
    </div>
  );
}

/* ══════════════════════════════════════ BUDGET VIEW ════════════════════
   Defined OUTSIDE App — has its own local state so typing doesn't
   re-render App and lose cursor focus.
═══════════════════════════════════════════════════════════════════════ */
function BudgetView({T,expenses,catData,monthlyTargets,saveMonthTarget,budgets,saveBudgets,total,curTarget,pct,isOver,selMonth,setSelMonth,budgetInput,setBudgetInput,catBudgetLocal,setCatBudgetLocal,handleBudgetSave,recurringTemplates,setRecurringTemplates,showToast,persist,user,isDark,pinVal,customCats}){
  const card=(e={})=>mkCard(T,e);
  const btn=(e={})=>mkBtn(e);
  const now=new Date();
  const curKey=monthKey(now);
  const years=Array.from({length:11},(_,i)=>2020+i);
  const [pickerYear,setPickerYear]=useState(parseInt(selMonth.split("-")[0])||now.getFullYear());
  ...
  /* use uncontrolled budget input via ref to prevent focus loss */
  const budgetRef=useRef(null);
  useEffect(()=>{ if(budgetRef.current) budgetRef.current.value=budgetInput; },[selMonth]);

  const onBudgetSave=async()=>{
    const v=parseFloat(budgetRef.current?.value||budgetInput);
    const raw=budgetRef.current?.value||budgetInput;
    if(!raw||raw.trim()===""){showToast("Enter an amount first.","warn");return;}
    ...
    <div style={{padding:"0 16px 124px"}}>
      ...
      <div style={{...card({marginBottom:16,background:T.gradSoft,borderColor:T.border})}}>
        ...
        <div style={{marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:6,overflowX:"auto",scrollbarWidth:"none",paddingBottom:4}}>
            {years.map(y=><button key={y} onClick={()=>{
              setPickerYear(y);
              const m=selMonth.split("-")[1];
              const newKey=`${y}-${m}`;
              setSelMonth(newKey);
              const existing=monthlyTargets[newKey]||0;
              setBudgetInput(existing>0?String(existing):"");
              if(budgetRef.current)budgetRef.current.value=existing>0?String(existing):"";
            }} style={{...btn({flexShrink:0,padding:"6px 12px",borderRadius:99,border:`1.5px solid ${pickerYear===y?T.a1:T.bdrSub}`,background:pickerYear===y?`${T.a1}18`:"transparent",color:pickerYear===y?T.a1:T.muted,fontSize:12,fontWeight:pickerYear===y?700:400})}}>{y}</button>)}
          </div>
        </div>
            ...
            {MONTHS.map((m,i)=>{
              ...
              return(
                <button key={k} onClick={()=>{
                  setSelMonth(k);
                  const existing=monthlyTargets[k]||0;
                  setBudgetInput(existing>0?String(existing):"");
                  if(budgetRef.current)budgetRef.current.value=existing>0?String(existing):"";
                }} style={{...btn({padding:"9px 4px",borderRadius:11,border:`1.5px solid ${isSel?T.a1:hasBudget?T.a3+"55":T.bdrSub}`,background:isSel?`${T.a1}18`:hasBudget?`${T.a3}08`:"transparent",color:isSel?T.a1:isCur?T.a3:T.muted,fontSize:11,textAlign:"center",fontWeight:isSel?700:isCur?600:400,position:"relative"})}}>
                  {m.slice(0,3)}
        ...
        {selTarget>0&&<div style={{marginBottom:14,padding:"10px 14px",borderRadius:12,background:T.isDark?"rgba(255,255,255,.05)":"rgba(0,0,0,.04)",border:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{fontSize:11,color:T.muted,marginBottom:3}}>Current for {monthLabel(selMonth)}</div><div style={{fontSize:20,fontWeight:700,color:T.a1,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(selTarget)}</div></div>
        ...
        {/* UNCONTROLLED input — uses ref so typing doesn't re-render App */}
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
          {PRESETS.map(v=><button key={v} onClick={()=>{if(budgetRef.current){budgetRef.current.value=String(v);budgetRef.current.focus();}setBudgetInput(String(v));}} style={{...btn({padding:"7px 12px",borderRadius:99,border:`1.5px solid ${budgetInput===String(v)?T.a1:T.bdrSub}`,background:budgetInput===String(v)?`${T.a1}18`:"transparent",color:budgetInput===String(v)?T.a1:T.muted,fontSize:12})}}>{fmt(v)}</button>)}
        </div>

        <button onClick={onBudgetSave} style={{...btn({width:"100%",padding:"16px",borderRadius:14,background:T.grad,color:"#fff",fontSize:16,boxShadow:`0 6px 24px ${T.glow}`,letterSpacing:.3})}}>
          ✓ Set Budget for {monthLabel(selMonth)}
        </button>
        {selTarget>0&&<button onClick={async()=>{await saveMonthTarget(selMonth,0);setBudgetInput("");if(budgetRef.current)budgetRef.current.value="";showToast("Budget cleared.");}} style={{...btn({width:"100%",padding:"10px",marginTop:10,border:`1px solid ${T.a4}30`,background:"transparent",color:T.a4,fontSize:13})}}>Clear budget for this month</button>}
      </div>
      ...
      {Object.keys(monthlyTargets).filter(k=>monthlyTargets[k]>0).length>0&&<div style={{...card({marginBottom:16})}}>
        ...
        {Object.entries(monthlyTargets).filter(([,v])=>v>0).sort((a,b)=>b[0].localeCompare(a[0])).map(([k,v])=>{
          ...
          const mTotal=mExp.reduce((s,e)=>s+e.amount,0); const mPct=Math.min((mTotal/v)*100,100); const mOver=mTotal>v;
      ...
      {recurringTemplates.length>0&&<div style={{...card({marginBottom:16})}}>
        ...
        {recurringTemplates.map(r=>(
          <div key={r.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,padding:"10px 12px",borderRadius:12,background:T.input,border:`1px solid ${T.bdrSub}`}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div><div style={{fontWeight:600,color:T.text,fontSize:13}}>{r.name}</div><div style={{fontSize:11,color:T.muted}}>Every {r.dayOfMonth}{r.dayOfMonth===1?"st":r.dayOfMonth===2?"nd":r.dayOfMonth===3?"rd":"th"} · {r.type}</div></div>
            </div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              <span style={{fontWeight:700,color:T.a1,fontFamily:"'JetBrains Mono',monospace",fontSize:13}}>{fmt(r.amount)}</span>
              <button onClick={async()=>{const updated=recurringTemplates.filter(x=>x.id!==r.id);setRecurringTemplates(updated);if(user)await persist(user.username,expenses,budgets,monthlyTargets,isDark,pinVal,customCats,updated);showToast("Template removed.");}} style={{...btn({width:26,height:26,borderRadius:7,border:`1px solid ${T.a4}30`,background:`${T.a4}08`,color:T.a4,fontSize:11})}}>✕</button>
            </div>
          </div>
        ))}
      </div>}
      ...
      <div style={{fontSize:12,color:T.muted,marginBottom:12}}>Set per-category spending limits. Tap outside to save.</div>
      {catData.map(cat=>{
        const bv=parseFloat(catBudgetLocal[cat.id])||0; const cp=bv>0?Math.min((cat.spent/bv)*100,100):0; const co=bv>0&&cat.spent>bv;
        return(<div key={cat.id} style={{...card({marginBottom:10,padding:"14px 16px"})}}>
    ...
    <div style={{padding:"0 16px 124px"}}>
      ...
      <div style={{...card({marginBottom:14,background:T.gradSoft,borderColor:T.border,padding:"20px"})}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          ...
          <div><div style={{fontFamily:"'DM Serif Display',serif",fontSize:20,color:T.text}}>{user.username}</div><div style={{fontSize:12,color:T.muted,marginTop:3}}>{expenses.length} expenses · {fmt(expenses.reduce((s,e)=>s+e.amount,0))} total · {recurringTemplates.length} recurring</div></div>
        </div>
      </div>
      <div style={{...card({marginBottom:14,padding:"20px"})}}>
        ...
        <div style={{fontSize:12,color:T.muted,marginBottom:14}}>Get alerts at 50%, 85% and 100% of budget.</div>
        <button onClick={async()=>{const ok=await requestNotifPermission();setNotifEnabled(ok);if(ok)showToast("Notifications enabled! 🔔");else showToast("Allow in browser settings.","warn");}} style={{...btn({width:"100%",padding:"13px",border:`1.5px solid ${Notification?.permission==="granted"?T.a3:T.border}`,background:Notification?.permission==="granted"?`${T.a3}12`:`${T.a1}08`,color:Notification?.permission==="granted"?T.a3:T.a1,fontSize:14})}}>{Notification?.permission==="granted"?"✅ Notifications Active":"Enable Notifications →"}</button>
      </div>
      <div style={{...card({marginBottom:14,padding:"20px"})}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          {[{m:true,l:"Dark",d:"Deep slate, warm gold",ico:"🌙"},{m:false,l:"Light",d:"Ivory, clean & bright",ico:"☀"}].map(({m,l,d,ico})=>(
            <button key={String(m)} onClick={()=>{setIsDark(m);if(user)persist(user.username,expenses,budgets,monthlyTargets,m,pinVal,customCats,recurringTemplates);}} style={{...btn({padding:"16px 12px",borderRadius:14,textAlign:"left",border:`1.5px solid ${isDark===m?T.a1:T.bdrSub}`,background:isDark===m?T.gradSoft:"transparent",boxShadow:isDark===m?`0 4px 18px ${T.glow}`:"none"})}}>
...
/* ════════════════════════════════════════════════════════════════════════
   MAIN APP — only state, logic, and shell here. No view definitions.
════════════════════════════════════════════════════════════════════════ */
export default function App() {
  ...
  const [showPwField,setShowPwField] = useState({password:false,confirm:false}); // renamed to avoid conflict
  ...
  /* budget tab top-level state (passed as props to BudgetView) */
  ...
  useEffect(()=>{
    ...
    else if(p>=85&&!notifSent.current["85"]){notifSent.current["85"]=true;setOverspendAlert({type:"warn",title:"⚠️ 85% Budget Used",msg:`Used ${p.toFixed(0)}% of ${fmt(curTarget)}. Only ${fmt(curTarget-total)} left.`});sendNotif("⚠️ Budget Warning",`${p.toFixed(0)}% used.`);}
    else if(p>=50&&!notifSent.current["50"]){notifSent.current["50"]=true;sendNotif("💡 Halfway","Half your monthly budget used.");}
  },[total,curTarget]);

  /* auto-add recurring templates */
  useEffect(()=>{
    if(!recurringTemplates.length||!user)return;
    ...
    recurringTemplates.filter(r=>r.active&&r.dayOfMonth===todayDay).forEach(async r=>{
      const alreadyAdded=expenses.some(e=>e.category===r.category&&e.amount===r.amount&&monthKey(new Date(e.date))===mk&&e.description===r.name);
      if(!alreadyAdded){
        ...
        await persist(user.username,newExp,budgets,monthlyTargets,isDark,pinVal,customCats,recurringTemplates);
  ...
  const doSignOut=()=>{
    ...
    setShowPwField({password:false,confirm:false});setPinVal("");setLocked(false);setPinMode("none");setPinInput("");setPinError("");
    notifSent.current={};
  };
  ...
  const addCustomCat=async()=>{
    ...
    if(user)await persist(user.username,expenses,budgets,monthlyTargets,isDark,pinVal,updated,recurringTemplates);
  ...
  const saveMonthTarget=async(key,amount)=>{
    const updated={...monthlyTargets,[key]:amount};setMonthlyTargets(updated);notifSent.current={};
    if(user)await persist(user.username,expenses,budgets,updated,isDark,pinVal,customCats,recurringTemplates);
  };
  ...
  const startVoice=()=>{
    ...
    r.onresult=async ev=>{
      ...
      const wordNums={ek:1,do:2,teen:3,char:4,paanch:5,chhe:6,saat:7,aath:8,nau:9,das:10,bis:20,sau:100,hazaar:1000,be:2,tran:3,chaar:4,panch:5,chha:6,nav:9,vees:20,one:1,two:2,three:3,four:4,five:5,six:6,seven:7,eight:8,nine:9,ten:10,twenty:20,thirty:30,forty:40,fifty:50,hundred:100,thousand:1000};
      let amount="";
      ...
      const cashW=["cash","nakad","nakdu","haath","note","rokda","roki","naqdee"];
  ...
  if(!user){
    ...
    /* FIX: eye button uses onMouseDown preventDefault to not steal focus from input */
    const togglePw=(field)=>{setShowPwField(p=>({...p,[field]:!p[field]}));};
    return(
      <div style={{minHeight:"100vh",width:"100%",background:T.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif",position:"relative",overflow:"hidden",padding:"32px 16px",transition:"background .3s"}}>
        ...
        <div style={{...mkCard(T,{padding:"32px 28px",width:"min(420px,100%)",zIndex:10,animation:"fadeUp .5s ease .1s both"})}}>
          ...
          {/* PASSWORD — FIX: onMouseDown preventDefault on eye button so it doesn't blur the input */}
          <div style={{marginBottom:isUp?16:24}}>
            ...
            <div style={{position:"relative"}}>
              <input type={showPwField.password?"text":"password"} value={aForm.password} placeholder="Minimum 6 characters"
              ...
              <button type="button"
                ...
                style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",padding:6,color:T.muted,fontSize:18,lineHeight:1,display:"flex",alignItems:"center",userSelect:"none"}}>
                {showPwField.password?"🙈":"👁️"}
              </button>
            </div>
          </div>
          {isUp&&<div style={{marginBottom:24}}>
            ...
            <div style={{position:"relative"}}>
              <input type={showPwField.confirm?"text":"password"} value={aForm.confirm} placeholder="Repeat password"
  ...
  return(
    <div style={{minHeight:"100vh",width:"100%",background:T.bg,color:T.text,fontFamily:"'DM Sans',sans-serif",position:"relative",maxWidth:480,margin:"0 auto",overflowX:"hidden",transition:"background .3s,color .3s"}}>
      <style>{css}</style>
      <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:0,overflow:"hidden"}}><div style={{position:"absolute",top:"-5%",right:"-10%",width:300,height:300,borderRadius:"50%",background:`radial-gradient(circle,${moodColor}${T.isDark?"10":"06"},transparent)`,transition:"background .5s"}}/><div style={{position:"absolute",bottom:"10%",left:"-8%",width:240,height:240,borderRadius:"50%",background:`radial-gradient(circle,${T.a3}${T.isDark?"08":"05"},transparent)`}}/></div>
      {/* Header */}
      ...
      <div style={{position:"relative",zIndex:1,paddingTop:14}}>
        ...
        {tab==="budget"&&<BudgetView T={T} expenses={expenses} catData={catData} monthlyTargets={monthlyTargets} saveMonthTarget={saveMonthTarget} budgets={budgets} saveBudgets={saveBudgets} total={total} curTarget={curTarget} pct={pct} isOver={isOver} selMonth={selMonth} setSelMonth={setSelMonth} budgetInput={budgetInput} setBudgetInput={setBudgetInput} catBudgetLocal={catBudgetLocal} setCatBudgetLocal={setCatBudgetLocal} handleBudgetSave={handleBudgetSave} recurringTemplates={recurringTemplates} setRecurringTemplates={setRecurringTemplates} showToast={showToast} persist={persist} user={user} isDark={isDark} pinVal={pinVal} customCats={customCats}/>}
